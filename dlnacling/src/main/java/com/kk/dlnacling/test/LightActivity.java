...
import com.kk.dlnacling.BrowserUpnpService;
import com.kk.dlnacling.R;
import com.kk.dlnacling.SwitchPower;
...
public class LightActivity extends Activity implements PropertyChangeListener, View.OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.btn_switchRouter).setOnClickListener(this);
    }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        menu.add(0, 0, 0, "switchRouter").setIcon(android.R.drawable.ic_menu_revert);
        menu.add(0, 1, 0, "toggleDebugLogging").setIcon(android.R.drawable.ic_menu_info_details);
        return true;
    }
        ...
        switch (item.getItemId()) {
            case 0:
                if (upnpService != null) {
                    ...
                    try {
                        if (router.isEnabled()) {
                            Toast.makeText(this, "disablingRouter", Toast.LENGTH_SHORT).show();
                            router.disable();
                        } else {
                            Toast.makeText(this, "enablingRouter", Toast.LENGTH_SHORT).show();
                            router.enable();
                        }
                    } catch (RouterException ex) {
                        Toast.makeText(this, "errorSwitchingRouter" + ex.toString(), Toast.LENGTH_LONG).show();
            ...
            case 1:
                ...
                if (logger.getLevel() != null && !logger.getLevel().equals(Level.INFO)) {
                    Toast.makeText(this, "disablingDebugLogging", Toast.LENGTH_SHORT).show();
                    logger.setLevel(Level.INFO);
                } else {
                    Toast.makeText(this, "enablingDebugLogging", Toast.LENGTH_SHORT).show();
    ...
    protected void setLightbulb(final boolean on) {
        runOnUiThread(new Runnable() {
            public void run() {
                ImageView imageView = findViewById(R.id.iv_light);
                imageView.setImageResource(on ? android.R.drawable.btn_star_big_on : android.R.drawable.btn_star_big_off);
